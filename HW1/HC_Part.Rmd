---
title: "STAT5703 HW1"
author: "Huang Chao, Wancheng Chen, Chengchao Jin"
output: pdf_document
---
...
$$(\hat{\lambda_M}+\frac{1}{2})^2=\frac{1}{n}\sum_{i=1}^{n}X_i^2+\frac{1}{4} \\ \hat{\lambda_M}=\frac{-1+\sqrt{1+\frac{4}{n}\sum_{i=1}^{n}X_i^2}}{2}$$
...
$$Eff(\lambda_{M1},\lambda_{M2})=\frac{MSE(\hat{\lambda_{M1}})}{MSE(\hat{\lambda_{M2})}}=\frac{Var(\hat{\lambda_{M1}})}{Var(\hat{\lambda_{M2})}}\\ \xrightarrow[n\rightarrow \infty]{\mathcal{D}}\mathcal{N}(0,\frac{\lambda}{\frac{4\lambda^3+6\lambda^2+1}{4\lambda^2+4\lambda+1}}=\mathcal{N}(0,\frac{4\lambda^3+4\lambda^2+\lambda}{4\lambda^3+6\lambda^2+1})<1$$
...
Corr
```
Negative sign, since the correlation is negative.
Without any calculation, I also expect the intercept to be positive and the slope to be negative. First, as age variable increases, the overall function of kidney, tot, tends to be lower and therefore the slope should be negative. Then, around age=20, the values of tot scatter around tot=0. So clearly, since the slope is negative, the value of tot should be larger than 0 at age=0. Intuitively, the overall function of kidney for a baby should be positive. So the intercept should be positive.
